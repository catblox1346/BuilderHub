...
local function writeKey(k)
	k = norm(k); if not k then return end
	ensureFolder()
	if isfile(OLD_KEY_FILE) then pcall(delfile,OLD_KEY_FILE) end
...
local function migrateOld()
	if isfile(OLD_KEY_FILE) then
		...
		if ok then _G.script_key = norm(v) or _G.script_key end
		pcall(delfile,OLD_KEY_FILE)
	end
end

local function clearKey()
	if isfile(KEY_FILE) then pcall(delfile,KEY_FILE) end
	_G.script_key = nil
	rawset(getfenv and getfenv() or _G,"script_key",nil)
end

local function set_global_key(v)
	v = norm(v); if not v then return end
	_G.script_key = v
...
if _G.script_key then
	...
	local rk = readKey()
	if rk then set_global_key(rk) end
end

local function validate(k)
	k = norm(k)
	if not k then return false,{code="KEY_INVALID",message="missing"} end
	set_global_key(k)
	local ok,status = pcall(function() return luarmor.check_key(k) end)
	if not ok then return false,{code="CHECK_FAILED",message=tostring(status)} end
	if status and status.code=="KEY_VALID" then writeKey(k) return true,status end
	return false,status
end
...
local function instcheck()
	...
	if ok then luarmor.load_script() return true end
	if status and status.code and status.code:find("KEY_") then clearKey() end
	return false
end

if instcheck() then return end

local win = ReGui:Window({Title = HUB_NAME.." | Key System", NoClose=true, NoResize=true, Size=UDim2.new(0,400,0,280)}):Center()
...
win:InputText({Label="Key", Placeholder="Paste key here", Default=keyText, Callback=function(_,v) keyText=v end})
...
rowA:Button({
	...
	Callback=function()
		...
		if ok then
			...
			task.delay(0.2,function() pcall(function() win:Close() end) end)
			luarmor.load_script()
		else
			if status and status.code and status.code:find("KEY_") then
				clearKey()
				local m = { KEY_HWID_LOCKED="Key locked to another HWID", KEY_INCORRECT="Incorrect key", KEY_INVALID="Invalid key" }
...
rowB:Button({
	...
	Callback=function()
		...
		p:Label({Text="1. Get Key and open link.\n2. Finish steps and copy the key.\n3. Paste and Submit.", TextWrapped=true})
		p:Separator()
